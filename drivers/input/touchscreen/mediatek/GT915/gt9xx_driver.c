...
extern s32 gtp_test_sysfs_init(void);    //add by caozhg
extern void gtp_test_sysfs_deinit(void); //add by caozhg
#ifndef MT6572
/*
...
#define GTP_CREATE_WR_NODE  (1)//add by caozhg
...
static s32 gtp_init_panel(struct i2c_client *client)
{
...
#endif
    {
        ...
        if (ret == SUCCESS)
        {
 		...
 		tpd_info.cid = send_cfg_buf[sensor_id][0];           
            if (opr_buf[0] < 90)
            {
...
static int gtp_info_read_proc(struct file *file, char *buffer, size_t count,  loff_t *ppos)
{
	...
	ptr = page;
	ptr += sprintf(ptr, "[Vendor]%s, [fw]0x%04x, [IC]GT915\n", manu_string, tpd_info.vid);
...
/*
    ...
    if (ret == SUCCESS)
    {
         GTP_DEBUG("IC Config Version: %d, 0x%02X",  opr_buf[0], opr_buf[0]);
    }
    else
    {
         GTP_ERROR("Failed to get ic config version!No config sent!");
...
*/
	...
	sprintf(temp_ver,"0x%04x",tpd_info.vid); 
	temp_ver_config = (char*)kmalloc(8,GFP_KERNEL);
	sprintf(temp_ver_config,"0x%02x",tpd_info.cid);


	switch(temp_pid)
	{
		case 0x00://No used
			devinfo_ctp_regchar("O-Film","O-Film",temp_ver,temp_ver_config,DEVINFO_USED);
			break;
			
		case 0x01:
			devinfo_ctp_regchar("shenyue","shenyue",temp_ver,temp_ver_config,DEVINFO_USED);
			break;
			
		case 0x02:
			devinfo_ctp_regchar("EACH","EACH",temp_ver,temp_ver_config,DEVINFO_USED);
			break;
			
		default:
			devinfo_ctp_regchar("unknown","unknown","unknown","unknown",DEVINFO_USED);
			break;
	}
...
static int __init tpd_driver_init(void)
{
    ...
    gtp_test_sysfs_init();//add by caozhg
    return 0;
}
