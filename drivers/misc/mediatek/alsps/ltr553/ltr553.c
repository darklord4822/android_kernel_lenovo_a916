...
#include "linux/mutex.h"//wangxiqiang
...
//#define APS_FUN_WXQ(f)               printk("wangxiqiang_l5930  %s %d \n", __FUNCTION__,__LINE__)
...
extern void mt_eint_registration(unsigned int eint_num, unsigned int pol, void (EINT_FUNC_PTR)(void), unsigned int is_auto_umask);
...
/*----------------------------------------------------------------------------*/
static DEFINE_MUTEX(read_lock);//wangxiqiang 
...
static int ltr559_i2c_read_reg(u8 regnum)
{
	...
	int res = 0;
	//APS_FUN_WXQ();
	mutex_lock(&read_lock);//wangxiqiang
	buffer[0]= regnum;
	...
	mutex_unlock(&read_lock);//wangxiqiang
	return reg_value[0];
}
...
static int ltr559_ps_set_thres_dynamic()
{
	u8 databuf[2];
	int res;
	struct ltr559_priv *obj = ltr559_obj;
...
static int ltr559_ps_enable(int gainrange)
{
	...
	//APS_FUN_WXQ();
	printk("wangxiqiang_l5930  %s %d  %d  \n", __FUNCTION__,__LINE__,gainrange);
	data = ltr559_i2c_read_reg(LTR559_PS_N_PULSES);
...
static int ltr559_als_enable(int gainrange)
{
	...
	data = ltr559_i2c_read_reg(LTR559_ALS_CONTR);
...
//	mt_eint_registration(CUST_EINT_ALS_NUM, CUST_EINT_ALS_DEBOUNCE_EN, CUST_EINT_ALS_POLARITY, ltr559_eint_func, 0);

	mt_eint_set_hw_debounce(CUST_EINT_ALS_NUM, CUST_EINT_TOUCH_PANEL_DEBOUNCE_CN);
    //mt_eint_registration(CUST_EINT_ALS_NUM, EINTF_TRIGGER_RISING, ltr559_eint_func, 1);
  //  mt_eint_registration(CUST_EINT_ALS_NUM, 1, ltr559_eint_func, 1);
...
static int ltr559_init_device(void)
{
	...
	/*set the register*/
	ltr559_i2c_write_reg(LTR559_PS_LED, 0x7f);
	ltr559_i2c_write_reg(LTR559_PS_N_PULSES, 0x0F);//wangxiqiang modify on 6.11
	ltr559_i2c_write_reg(LTR559_PS_MEAS_RATE, 0x0);//wangxiqiang  10ms  //50ms
	...
	{	
		...
		ltr559_i2c_write_reg(LTR559_INTERRUPT_PERSIST, 0x40);//wangxiqiang

	}
	mdelay(WAKEUP_DELAY);//wangxiqiang 
	return 0;
...
/*singal interrupt function add*/
		...
		printk("wangxiqiang_l5930 %d",intr_flag_value);
		if(intr_flag_value){
				...
				//APS_FUN_WXQ();
					/*added by WeeLiat 20140402*/
				if(dynamic_cali > 20 && obj->ps < (dynamic_cali - 50))
				{
				...
				/*the ended added by WeeLiat20140402*/
...
static int ltr559_i2c_resume(struct i2c_client *client)
{
	...
	if (1 == ltr559_power(obj->hw, 1)) {
		if (err = ltr559_init_device()) {
			APS_ERR("initialize client fail!!\n");
			return err;
		}
		if (obj->ps_enable) {
			if (err = ltr559_ps_enable()) {
				APS_ERR("enable ps fail: %d\n",err);
			}
			return err;
		}
