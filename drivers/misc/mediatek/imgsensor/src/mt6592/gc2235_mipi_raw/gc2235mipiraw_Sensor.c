...
#ifdef GC2235_DEBUG
//	#define printk(fmt, arg...) printk("[GC2235Raw] ",  fmt, ##arg)
#else
	#define printk(fmt, arg...)
#endif

#ifdef GC2235_DEBUG_SOFIA
	#define printkSOFIA(fmt, arg...) printk("[GC2235Raw] ",  fmt, ##arg)
#else
	#define printkSOFIA(fmt, arg...)
#endif
...
static kal_uint8 GC2235_read_cmos_sensor(kal_uint8 addr)
{
  ...
  out_buff[0] = addr;
    ...
    if (0 != iReadRegI2C((u8*)out_buff , 1, (u8*)in_buff, 1, GC2235MIPI_WRITE_ID)) {
        printk("ERROR: GC2235MIPI_read_cmos_sensor \n");
    }
  return in_buff[0];
}
...
static void GC2235_SetDummy( const kal_uint32 iPixels, const kal_uint32 iLines )
{
	...
	printk("GC2235_SetDummy framelength=%d\n",vb+GC2235_VALID_LINE_NUMS);
...
UINT32 GC2235GetSensorID(UINT32 *sensorID)
{
	...
	printk("GC2235GetSensorID enter :\n ");
...
//while(1)
{
    ...
    *sensorID =((GC2235_read_cmos_sensor(0xf0) << 8) | GC2235_read_cmos_sensor(0xf1)); 
	...
	s_DEVINFO_ccm->device_module = "G7P2-AE712HQ_V3";	//can change if got module id
	s_DEVINFO_ccm->device_vendor = "Huaquan";
	s_DEVINFO_ccm->device_ic = "GC2235";	
	s_DEVINFO_ccm->device_version = "GalaxyCore";
...
void GC2235_SetShutter(kal_uint32 iShutter)
{
	...
	{
		//printk("always UPDATE SHUTTER when gc2235.sensorMode == SENSOR_MODE_CAPTURE\n");
	}
	else{
		if(gc2235.sensorMode == SENSOR_MODE_CAPTURE)
		{
			//printk("capture!!DONT UPDATE SHUTTER!!\n");
			//return;
		}
	}
   ...
   printk("GC2235_SetShutter:%x \n",iShutter); 
   	
 //  printk("GC2235_SetShutter 0x03=%x, 0x04=%x \n",GC2235_read_cmos_sensor(0x03),GC2235_read_cmos_sensor(0x04));
   return;
}   /*  GC2235_SetShutter   */
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	spin_unlock(&gc2235mipiraw_drv_lock);
	GC2235SetFlipMirror(GC2235_ORIENTATION);
	printk("GC2235Video mirror:%d\n",sensor_config_data->SensorImageMirror);

	//printkSOFIA("[GC2235Video]frame_len=%x\n", ((GC2235_read_cmos_sensor(0x380e)<<8)+GC2235_read_cmos_sensor(0x380f)));

	printk("GC2235Video exit:\n");
    return ERROR_NONE;
}
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	{
		printk("GC2235Capture BusrtShot!!!\n");
	}else{
		printk("GC2235Capture enter:\n");
		...
		printk("[GC2235Capture]gc2235.shutter=%d, read_pv_shutter=%d, read_pv_gain = 0x%x\n",gc2235.shutter, shutter,gc2235.sensorGlobalGain);
		...
		//printk("[GC2235Capture] mirror&flip: %d\n",sensor_config_data->SensorImageMirror);
		
		printk("GC2235capture mirror:%d\n",sensor_config_data->SensorImageMirror);
		GC2235SetFlipMirror(GC2235_ORIENTATION);
		GC2235_SetDummy(gc2235.DummyPixels,gc2235.DummyLines);

	    if(GC2235CurrentScenarioId==MSDK_SCENARIO_ID_CAMERA_ZSD)
	    {
			printk("GC2235Capture exit ZSD!!\n");
			return ERROR_NONE;
	    }
		printk("GC2235Capture exit:\n");
	}
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
		...
		spin_unlock(&gc2235mipiraw_drv_lock);
		printk("GC2235CurrentScenarioId=%d\n",GC2235CurrentScenarioId);
    switch (ScenarioId)
    {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
...
UINT32 GC2235SetVideoMode(UINT16 u2FrameRate)
{
    ...
    printk("[GC2235SetVideoMode] frame rate = %d\n", u2FrameRate);
	...
	if(u2FrameRate==0)
	{   
		printk("Disable Video Mode or dynimac fps\n");	
		spin_lock(&gc2235mipiraw_drv_lock);
		gc2235.DummyPixels = 0;//define dummy pixels and lines
	...
	if(u2FrameRate >30 || u2FrameRate <5)
	    printk("error frame rate seting %fps\n",u2FrameRate);
    ...
    if(gc2235.sensorMode == SENSOR_MODE_VIDEO)//video ScenarioId recording
    {
		...
		spin_unlock(&gc2235mipiraw_drv_lock);
		printk("GC2235SetVideoMode MIN_Frame_length %d\n",MIN_Frame_length);
...
{
	//return ERROR_NONE;
    //printk("[GC2235SetAutoFlickerMode] frame rate(10base) = %d %d\n", bEnable, u2FrameRate);
...
{
    printk("[GC2235SetTestPatternMode] Test pattern enable:%d\n", bEnable);
    if(bEnable)
    {
...
UINT32 GC2235MIPISetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) {
	...
	printk("GC2235MIPISetMaxFramerateByScenario: scenarioId = %d, frame rate = %d\n",scenarioId,frameRate);
	switch (scenarioId) {
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
